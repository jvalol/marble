...
impl Platform {
    ...
    pub fn is_standing_on(&self, position: Vec3) -> bool {
        let feet = Sphere::new(position - Vec3::Y * marble::GROUND_REACH, marble::RADIUS);
...
impl Course {
    ...
    pub fn on_goal(&self, position: Vec3) -> bool {
        self.platforms.iter().any(|platform| {
            platform.kind == PlatformKind::Goal && platform.is_standing_on(position)
        })
    }
