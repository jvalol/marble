...
mod tests {
  ...
  fn a_fall_costs_time() {
    ...
    assert!((run.time - (1.0 + FALL_PENALTY)).abs() < 1e-6, "time {}", run.time);
  }
  ...
  fn a_fall_returns_to_the_checkpoint() {
    ...
    assert!((marble.position - checkpoint).length() < 1e-3, "{:?}", marble.position);
    assert_eq!(marble.velocity, glam::Vec3::ZERO, "it should arrive at rest");
  }
